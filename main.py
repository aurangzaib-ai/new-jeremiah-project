...
def show_sidebar():
    ...
    with st.sidebar:
        ...
        # Temporarily disabled until file created:
...
def show_main_dashboard():
    ...
    with col1:
        ...
        The system uses AI to analyze applicant data including credit scores, driving history, vehicle information, and coverage requirements to make intelligent underwriting decisions.
        """)
    with col2:
        st.markdown("### Quick Actions")
        if st.button("Start Evaluation", use_container_width=True, type="primary"):
            st.switch_page("pages/02_Evaluate.py")
        # Disabled until file created
        # if st.button("A/B Testing", use_container_width=True):
        #     st.switch_page("pages/03_AB_Testing.py")
        if st.button("Configuration", use_container_width=True):
            st.switch_page("pages/04_Configuration.py")
